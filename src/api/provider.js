...
export default class Provider {
  ...
  getTasks() {
    ...
    const storeTasks = Object.values(this._store.getItems()).reverse();
  ...
  sync() {
    if (isOnline()) {
      ...
      return this._api.sync(storeTasks)
        .then((response) => {
          const createdTask = getSyncedTasks(response.created);
          const updatedTask = getSyncedTasks(response.updated);

          const items = createStoreStructure([...createdTask, ...updatedTask]);
